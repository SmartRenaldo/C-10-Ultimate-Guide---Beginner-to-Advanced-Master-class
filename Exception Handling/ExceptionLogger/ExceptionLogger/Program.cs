namespace ExceptionLoggerExample
{
    class BankAccount
    {
        ...
        public void Transfer(BankAccount sourceAccount, BankAccount destinationAccount, double amount)
        {
            try
            {
                if (amount < 0 || amount > 100000)
                    throw new ArgumentOutOfRangeException("amount", 100000, "For funds transfer, the value of 'amount' should be between 1 to 100000");
    ...
    class Program
    {
        static void Main()
        {
            try
            {
                BankAccount bobBankAccount = new BankAccount() { AccountNumber = 101, AccountHolderName = "Bob", CurrentBalance = 100000 };
