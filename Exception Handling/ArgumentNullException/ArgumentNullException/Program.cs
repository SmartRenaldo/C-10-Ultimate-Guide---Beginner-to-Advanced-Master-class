...
namespace ArgumentNullExceptionExample
{
    ...
    class Program
    {
        static void Main()
        {
            try
            {
                BankAccount scottBankAccount = new BankAccount() { AccountNumber = 101, AccountHolderName = "Bob", CurrentBalance = 10000 };
                BankAccount gandhiBankAccount = new BankAccount() { AccountNumber = 102, AccountHolderName = "Alice", CurrentBalance = 5000 };
                BankAccount stevenBankAccount = null;

                FundsTransfer fundsTransfer = new FundsTransfer();
                fundsTransfer.Transfer(scottBankAccount, stevenBankAccount, 1000);
